...
def main():
    ...
    quality_factor_list = [90,70]
    testset_name = 'DIV2K_train_HR_mini_double_compressed_90_gray' # 'LIVE1_gray' 'Classic5' 'BSDS500_gray'
    n_channels = 1            # set 1 for grayscale image, set 3 for color image
    model_name = 'fbcnn_gray_double.pth'
    nc = [64,128,256,512]
    nb = 4
    show_img = False                 # default: False
    testsets = '/Users/ceabyfernandez/bachelorsthesis/Datasets/DIV2K' 
    results = 'test_results_doublecompressed_gray'    
    ...
    for qf1 in quality_factor_list:
        for qf2 in quality_factor_list:
            ...
            set_start_time = time.time() #track time
            for idx, img in enumerate(H_paths):
                img_start_time = time.time()  # Start timer per image
                ...
                test_results['times'].append(img_elapsed)
                logger.info('{:s} - PSNR: {:.2f} dB; SSIM: {:.3f}; PSNRB: {:.2f} dB; Time: {:.2f}s.'.format(img_name+ext, psnr, ssim, psnrb, img_elapsed))
            ...
            ave_time = sum(test_results['times']) / len(test_results['times'])

            logger.info(
                'Average PSNR/SSIM/PSNRB - {} -: {:.2f}$\\vert${:.4f}$\\vert${:.2f}.'.format(result_name+'_qf1_'+str(qf1)+'_qf2_'+str(qf2), ave_psnr, ave_ssim, ave_psnrb))
            logger.info('Average time per image: {:.2f}s'.format(ave_time))
            logger.info('Total elapsed time for set: {:.2f}s ({:.2f} min)'.format(set_elapsed, set_elapsed/60))
