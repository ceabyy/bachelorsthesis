...
def main():
    ...
    testset_name = 'DIV2K_train_HR_mini_compressed_90'  # folder name of real images
    ...
    QF_control = [90] # adjust qf as input to provide different results
    ...
    start_set_time = time.time()  # <--- start timing for the whole set
    ...
    # calculate and log average time per set
    elapsed_set_time = time.time() - start_set_time
    avg_time_per_image = elapsed_set_time / len(L_paths)
    logger.info('Total time for the set: {:.4f}s, Average time per image: {:.4f}s'.format(elapsed_set_time, avg_time_per_image))
