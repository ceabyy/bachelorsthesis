# python statistics_generator_oapt.py
...
gt_folder = "/Users/ceabyfernandez/bachelorsthesis/statistics_doublecompressed/groundtruth"
lq_folder = "/Users/ceabyfernandez/bachelorsthesis/statistics_doublecompressed/fbcnn_restored/9070"
...
# Define crop if OAPT removes pixels
crop = 4  # adjust if OAPT removes a different amount
for gt_name, lq_name in zip(gt_images, lq_images):
    ...
    # Crop GT to match LQ dimensions (OAPT removes 4 pixels per border)
    h_lq, w_lq = img_lq.shape[:2]
    img_gt_c = img_gt[crop:crop+h_lq, crop:crop+w_lq, :]

    # Convert to float32 in range [0,1]
    img_gt_f = img_gt_c.astype(np.float32) / 255.0
...
output_path = os.path.join(lq_folder, "9070_statistics.csv")
